<template>
  <div
    class="component whiteborb"
    :style="toolSelected === tools.Pan ? 'cursor: move;' : ''"
  >
    <action-menu
...
export default class Whiteborb extends Vue {
  ...
  private async addMathField(mouse: { x: number; y: number }) {
    if (this.isTypingMath) {
      ...
      const field = MathLive.makeMathField(`mathfield-${this.nMathFields}`, {
  ...
  private pushSnapshot() {
    this.currentSnapshotIndex += 1;
    if (this.canRedo) {
      this.snapshotHistory = this.snapshotHistory.slice(
        0,
